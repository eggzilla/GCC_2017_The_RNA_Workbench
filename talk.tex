...
\author[RNA Workbench Team]{Florian Eggenhofer, B\'{e}r\'{e}nice Batut, J\"org Fallmann, Andrea Bagnacani, Peter F. Stadler, Rolf Backofen and the RNA Workbench Team}
...
\frame{
  ...
  \end{figure}
  \item<4-> Currently six workflows in the \textit{RNA} workbench%TODO - add workflows
    \begin{itemize}
    \item<5-> \only<5-9>{\textit{ncRNA} analysis}\only<10>{\textcolor{red}{\textit{ncRNA} analysis}}
    \item<6-> \only<6-9>{\textit{RNA} model construction}\only<10>{\textcolor{red}{\textit{RNA} model construction}}
...
\frame{
  ...
  Workbench is a joint effort of the european \textit{ncRNA} community\\
  \vspace{0.5cm}
  \begin{itemize}
  \item<2-> \textbf{12} contributors \textit{RNA}-workbench\\
  \item<3-> \textbf{54} contributors galaxytools\\
  \end{itemize}
  \vspace{0.5cm}
  \onslide<4-> Benefits from collaboration and integration ?\\  
  \vspace{0.5cm}
